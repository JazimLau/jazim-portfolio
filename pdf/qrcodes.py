...
切换站点来源（环境变量 QR_ORIGIN 覆盖默认值）：
  - 当前默认 = GitHub Pages（国内无代理实测可达 200 OK；vercel.app 已被国内 DNS 污染打不开）：
      https://jazimlau.github.io/jazim-portfolio
  - 待正式域名 jazimportfolio.com 配置生效后，改回：
      $env:QR_ORIGIN="https://jazimportfolio.com"; python pdf/qrcodes.py
  注意：GitHub Pages 为仓库子路径部署，ORIGIN 必须带 /jazim-portfolio/ 前缀；
  正式域名/Vercel 为根路径，不带前缀。
"""
import os
import qrcode
...
ORIGIN = os.environ.get("QR_ORIGIN", "https://jazimlau.github.io/jazim-portfolio")
